...
struct NearestFilterView: View {
    @StateObject private var viewModel = NearestFilterViewModel()
    @State private var smokingAreas: [SmokingArea] = [] 
    
    var body: some View {
        VStack {
            if let userLocation = viewModel.userLocation {
                Text("User Location: \(userLocation.coordinate.latitude), \(userLocation.coordinate.longitude)")
                    .padding()
            }
            ...
            if smokingAreas.isEmpty {
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 10) {
                        ForEach(viewModel.distances.keys.sorted(by: { viewModel.distances[$0]! < viewModel.distances[$1]! }), id: \.self) { id in
                            if let area = smokingAreas.first(where: { $0.persistentModelID == id }) {
                                let distance = viewModel.distances[area.persistentModelID]
                                SmokingAreaListItem(area: area, distance: distance)
                            }
                        }
                    }
                    .padding()
        ...
        .onAppear {
            viewModel.requestUserLocationAndCalculateDistances(smokingAreas: smokingAreas)
        }
    }
}
